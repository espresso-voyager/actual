...
- **Sidebar:** grouping is fully decoupled from on/off-budget status AND currency. Order: **On budget** (unsectioned budgeted accounts) → **Investments** (unsectioned off-budget) → **named groups** (alphabetical), which can hold ANY account regardless of budget status. Groups come from the per-account "Set section" control or the **"Add group"** button at the bottom of the sidebar (empty groups persist via the `sidebar-groups` pref). Group headers show a **live summed balance** (custom binding `accountSetBalance` in `spreadsheet/bindings.ts`). Every account row carries a **(¥)/($)** badge; the currency is toggled by the **¥/$ pill** on the account header (`usd-account-*` pref, also feeds the JPY valuation pill and Net Wealth card). Caveat: keep USD accounts off-budget — a USD on-budget account would feed raw dollar amounts into envelope math.
