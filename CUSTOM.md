...
- **Group management:** the `sidebar-groups` pref is the group ORDER — drag a group header onto another header to reorder whole groups. Right-click a group header → **Rename group**: named groups are truly renamed (order list + member prefs migrate); On budget / Investments keep their identity and get a display-label override (`sidebar-group-labels` pref, also used for the /accounts/onbudget and /accounts/offbudget page titles). **"+ Add group"** lives at the sidebar bottom, directly above "+ Add account".
- **Quick delete on transactions** (`TransactionsTable.tsx`): every committed transaction row has a hover-visible trash button (with a matching header spacer) that deletes via the same `onBatchDelete` path as the right-click menu — split handling and undo (Cmd+Z) included.
- **Group click-through pages:** clicking any sidebar group header opens All Accounts pre-filtered to exactly that group's member accounts (`filterConditions` with `account oneOf`, `filterName` = group label, passed as navigation state through a new `state` prop on internal `Link`s). This replaces the old links to /accounts/onbudget and /accounts/offbudget, which ignore grouping and showed non-member accounts' transactions.
